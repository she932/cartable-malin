...
async function extractItemsFromImage(base64, mediaType) {
  ...
  if (!data.items || !Array.isArray(data.items)) throw new Error("Aucun article détecté sur ce document.");
...
export default function App() {
  ...
  function onPickPhoto(e) {
    ...
    if (file.type === "application/pdf") {
      fileToRawBase64(file)
        .then((b64) => {
          setPendingBase64(b64);
          setPendingMediaType("application/pdf");
      ...
      fileToResizedBase64(file)
        .then((b64) => {
          ...
          setPendingFileName(file.name);
          setPendingPreview(`data:image/jpeg;base64,${b64}`);
        })
        .catch((err) => setError(err.message));
    }
    e.target.value = "";
  ...
  async function confirmScan() {
    ...
    setError("");
    try {
      const items = await extractItemsFromImage(pendingBase64, pendingMediaType);
            ...
            {!pendingBase64 && (
              ...
              >
                ...
                <p className="text-sm max-w-xs" style={{ color: "#5B6B85" }}>
                  Photo, image depuis ta galerie, ou PDF — pour <b>{activeChild.name}</b>, une matière à la fois ou la
                  liste entière.
                </p>
                <input ref={fileInputRef} type="file" accept="image/*,application/pdf" className="hidden" onChange={onPickPhoto} />
              </div>
            )}
            ...
            {pendingBase64 && (
              <div className="rounded-2xl bg-white p-4 border" style={{ borderColor: "#C7DCEC" }}>
                {pendingMediaType === "application/pdf" ? (
                  <div className="flex items-center gap-3 rounded-lg p-3 mb-3" style={{ background: "#F4F6F8" }}>
                    <FileText size={28} color="#D64545" className="shrink-0" />
                    <p className="text-sm truncate" style={{ color: "#24324A" }}>
                      {pendingFileName || "Document PDF"}
                    </p>
                  </div>
                ) : (
                  <img src={pendingPreview} alt="Aperçu de la liste" className="w-full max-h-64 object-contain rounded-lg mb-3" style={{ background: "#F4F6F8" }} />
                )}
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {loading ? "Lecture en cours…" : "Analyser la liste"}
                  </button>
...
import React, { useState, useRef, useEffect } from "react";
import { Camera, Plus, X, Trash2, Users, ShoppingCart, Loader2, Check, Pencil, Sparkles, AlertCircle, ChevronRight, Tag } from "lucide-react";
...
async function extractItemsFromImage(base64) {
  const response = await fetch("/api/extract", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || "Le service de lecture a répondu une erreur.");
  if (!data.items || !Array.isArray(data.items)) throw new Error("Aucun article détecté sur cette photo.");
  return data.items.filter((it) => it.name && it.name.trim());
}
...
export default function App() {
  ...
  async function confirmScan() {
    ...
    setError("");
    try {
      const items = await extractItemsFromImage(pendingBase64);
      const subj = pendingSubject.trim();
      const scanId = uid();
      const newItems = items.map((it) => ({
        id: uid(),
